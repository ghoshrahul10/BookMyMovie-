...
def book_show(request,show_id):
  ...
  if request.method == "POST":
    selected_seats= request.POST.get("selected_seat")
    if not selected_seats:
      return render(request,"./pmax/seat_selection.html",{"show":show, "booked_seats":booked_seats, "seats":seats, "error":"Please select at least one seat."})
    seats_list = selected_seats.split(",")
    if len(seats_list) > show.available_seats:
    ...
    total_price = len(seats_list) * show.price
    user = User.objects.get(id = user_id)
    ...
    show.available_seats -= len(seats_list)
    show.save()
    return render(request,"./pmax/sucess.html",{
    "show": show,
    "seats": seats_list,
    "total": total_price
    })
